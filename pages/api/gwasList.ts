...
export default async function handler(
  ...
  try {
    ...
    if (table === 'gwasColorFruit') {
      const [list, total] = await Promise.all([
        db.gwasColorFruit.findMany({ skip, take: Number(size), where, orderBy: { no: 'asc' } }),
        db.gwasColorFruit.count({ where: where }),
      ]);
    ...
    if (table === 'gwasColorPeel') {
      const [list, total] = await Promise.all([
        ...
        db.gwasColorPeel.count({ where: where }),
      ]);
      return res.json({ success: true, data: list as any, total });
    }
    if (table === 'gwasFiberFruit') {
      const [list, total] = await Promise.all([
        db.gwasFiberFruit.findMany({ skip, take: Number(size), where, orderBy: { no: 'asc' } }),
        db.gwasFiberFruit.count({ where: where }),
      ]);
      return res.json({ success: true, data: list as any, total });
    }
    if (table === 'gwasFiberFeer') {
      const [list, total] = await Promise.all([
        db.gwasFiberFeer.findMany({ skip, take: Number(size), where, orderBy: { no: 'asc' } }),
        db.gwasFiberFeer.count({ where: where }),
      ]);
      return res.json({ success: true, data: list as any, total });
    }
    if (table === 'gwasFirmnessFruit') {
      const [list, total] = await Promise.all([
        db.gwasFirmnessFruit.findMany({ skip, take: Number(size), where, orderBy: { no: 'asc' } }),
        db.gwasFirmnessFruit.count({ where: where }),
      ]);
      return res.json({ success: true, data: list as any, total });
    }
    if (table === 'gwasFirmnessPeer') {
      const [list, total] = await Promise.all([
        db.gwasFirmnessPeer.findMany({ skip, take: Number(size), where, orderBy: { no: 'asc' } }),
        db.gwasFirmnessPeer.count({ where: where }),
      ]);
      return res.json({ success: true, data: list as any, total });
    }
    if (table === 'gwasFuzz') {
      const [list, total] = await Promise.all([
        db.gwasFuzz.findMany({ skip, take: Number(size), where, orderBy: { no: 'asc' } }),
        db.gwasFuzz.count({ where: where }),
      ]);
      return res.json({ success: true, data: list as any, total });
    }
    if (table === 'gwasOrgan') {
      const [list, total] = await Promise.all([
        db.gwasOrgan.findMany({ skip, take: Number(size), where, orderBy: { no: 'asc' } }),
        db.gwasOrgan.count({ where: where }),
      ]);
      return res.json({ success: true, data: list as any, total });
    }
