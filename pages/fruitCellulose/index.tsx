...
const Metabolism = () =>
{
  ...
  const [activeKey, setActiveKey] = useState('gwasFiberFruit')
  // tab分页
  const items = [
    {
      key: 'gwasFiberFruit',
      label: t.grape_skin_cellulose
    }, {
      key: 'gwasFiberFeer',
      label: t.grape_flesh_cellulose
    }]
  const onChange = (key: string) => {
    setActiveKey(key)
  }
  const fiberFruitColumns = [
  ...
  const [activeColumn, setActiveColumn] = useState(fiberFruitColumns);
  useEffect(() => {
    ...
    setActiveColumn(activeKey === 'gwasFiberFruit' ? fiberFruitColumns : fiberPeerColumns);
    fetchGrapeData();
